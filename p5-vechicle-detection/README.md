...
Please find all the code in Jupyter Notebook. The code is splitted in the same sections as this writeup and could be easily found.
...
Dataset looks not so big, but may be enough for good classification. 
...
1. First of all, quality of the model should be improved. Current model gives lots of false positives and probably I should use different model, or extend train dataset, or use different features from the image to detect. I did not add filtering of false positives by adding of cross frame checks, where rectangles on one frame will be considered as wrong detections if they do not exist on previous ones, because do not think it will be safe from driving perspective if there would be fast objects that appear in front of the car. 
...
From my perspective vehicle detection should be done on sensor level — it will be probably faster and more robust.